...
public class BodyContentImpl extends BodyContent {
    ...
    void setWriter(Writer writer) {
        ...
        closed = false;
        if (writer != null) {
            // 根据规范, JspContext.pushBody(java.io.Writer writer)返回的JspWriter要表现得好像无缓冲.
        	// 意味着它的getBufferSize()总是返回 0. JspWriter.getBufferSize()的实现返回JspWriter的'bufferSize'字段的值, 由这个类继承的. 
            // 因此, 只是保存当当前'bufferSize' (所以我们稍后可以恢复它 BodyContentImpl, 如果重复调用 PageContext.pushBody()), 在它设置为 0之前.
            if (bufferSize != 0) {
